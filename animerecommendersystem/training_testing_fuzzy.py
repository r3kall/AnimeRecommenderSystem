"""
This file will perform training and testing on the recommendation system based on fuzzy clustering.
...
=> HOW DO WE TUNE?
    => Our system has three main parameters:
        ...
        - Number of clusters(C)            => Possible values: [10, 15, 20]
...
=> ... CHE VOR DI?
    ...
    => Select one group as testing set and the other four as training set, in a round-robin fashion.
        => COME SE FA ER TRAINING?
            =>  We define a parameter P as a tuple P=[N, C, R], and for each P, for each user U in the training set,
...
NOTE: we need to create one user_cluster_matrix and one item_cluster_matrix for each tuple.
"""
